...
const homeModalMarkImg =
  homeModalSecondMain.querySelector("#homeModal_markImg");
...
const itemModalMainImg =
  itemModalMainImgContainer.querySelector(".itemModal_img");
...
const itemModalResultImg =
  itemModalResultImgContainer.querySelector(".itemModal_img");
...
const makePartClassNameBtn =
  makePartClassNavBtn.querySelector(".classNav_nameBtn");
const makePartGradeNavBtn = makePart.querySelector("#makeGradeNavBtn");
const makePartGradeNameBtn =
  makePartGradeNavBtn.querySelector(".classNav_nameBtn");
...
const craftTargetNameContainer =
  craftExecuter.querySelector("#craftTargetName");
...
const mySellingItemContainers =
  currentSaleInventory.querySelectorAll(".itemContainer");
const marketClassNavContainer = marketPart.querySelector(".classNav");
const marketSalePartNavBtns =
  marketClassNavContainer.querySelectorAll(".classNav_navBtn");
const marketBuyModeBtn =
  marketClassNavContainer.querySelector("#marketBuyModeBtn");
...
const marketItemDetailIdContainer =
  marketItemDetailContainer.querySelector(".itemId");
const marketItemDetailCodeContainer =
  marketItemDetailContainer.querySelector(".itemCode");
...
const cashItemDetailCodeContainer =
  cashItemDetailContainer.querySelector(".itemCode");
...
const cashItemDetailDescriptionContainer =
  cashItemDetailContainer.querySelector(".itemDetailContainer_itemDescription");
...
const profilePasswordContainer =
  profileModal.querySelector("#profile_password");
...
const getCreatorInfoAtFirst = () => {
  const firstCreatorStringInfoContainer =
    document.querySelector(".firstCreatorInfo");
...
const getTotemsInfoAtFirst = () => {
  const firstTotemsStringInfoContainer =
    document.querySelector(".firstTotemsInfo");
...
const getMyItemsInfoAtFirst = () => {
  const firstMyItemsStringInfoContainer =
    document.querySelector(".firstItemInfo");
...
const resetItemContainers = (itemClass) => {
  ...
  const itemPartContainers =
    targetItemPartInventories.querySelectorAll(".itemContainer");
  const marketPartContainers =
    targetMarketPartInventories.querySelectorAll(".itemContainer");
...
const renderItems = (itemClass) => {
  const targetItemContainers =
    itemPartInventories[itemClass - 1].querySelectorAll(".itemContainer");
  const targetSaleItemContainers =
    marketPartInventories[itemClass - 1].querySelectorAll(".itemContainer");
  const targetClass = myItems[itemClass - 1];
  let i = 0;
  ...
  switch (partIndex) {
    ...
    case 1: {
      ...
      marketItemDetailIdContainer.innerText = idContainer
        ...
        : "";
      const itemSaleCodeContainer =
        itemContainer.querySelector(".itemSaleCode");
...
const equipMark = (code) => async () => {
  try {
    ...
    itemModal.style.display = "none";
    OutOfItemModal.style.display = "none";
    const res = await axios.default.post(`/summoner/mark/${summonerIndex}`, {
      code,
    });
    const { data } = res;
    const { fatal } = data;
...
const disassembleItem = (code) => async () => {
  try {
    ...
    const res = await axios.default.post(`/item/disassemble/${code}`, {
      amounts,
    });
    const { data } = res;
    const { fatal } = data;
...
const enhanceMark = async () => {
  try {
    ...
    const markCodeContainer = Array.from(
      makeInventory.querySelectorAll(".itemCode")
    ).filter((itemCode) => itemCode.innerText === code)[0];
